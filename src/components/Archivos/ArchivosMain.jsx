...
import { GetMedidas } from '../formularios/GetFormularios/GetMedidas';
import { GetPeligros } from '../formularios/GetFormularios/GetPeligros';
import { GetProteccion } from '../formularios/GetFormularios/GetProteccion';
import { GetRiesgos } from '../formularios/GetFormularios/GetRiesgos';
import Swal from 'sweetalert2';

export const ArchivosMain = () => {
  const [formularios, setFormularios] = useState([]);
  const {id} = useParams()
  const [checkboxValues, setCheckboxValues] = useState({checkbox1: false,checkbox2: false,checkbox3: false,checkbox4: false,checkbox5: false,
  checkbox6: false,checkbox7: false,checkbox8: false,checkbox9: false,checkbox10: false,checkbox11: false,checkbox12: false
  });

  const [textArea, setTextArea] = useState({textArea1: '',textArea2: '',textArea3: '',textArea4: '',textArea5: '',
    textArea6: '',textArea7: '',textArea8: '',textArea9: '',textArea10: '',
    textArea11: '',textArea12: '',textArea13: '',textArea14: '',textArea15: '',textArea16: ''
  })
  ...
  const obtenerFormularios = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        filtrarFormularios(response.data, filtro);
      }
    } catch (error) {
  ...
  const cambiarFiltro = nuevoFiltro => {
    ...
    filtrarFormularios(formularios, nuevoFiltro); // Filtra los formularios según el nuevo estado
  };
  const tableStyle = {
    opacity: 0.7,
    cursor: "not-allowed",
  ...
  const filtrarFormularios = (formularios, estado) => {
      const formulariosFiltrados = formularios.filter(formulario => formulario.estado === estado);
      setFormulariosFiltrados(formulariosFiltrados);
  ...
  <symbol id="geo-fill" viewBox="0 0 16 16">
    <path fill-rule="evenodd" d="M4 4a4 4 0 1 1 4.5 3.969V13.5a.5.5 0 0 1-1 0V7.97A4 4 0 0 1 4 3.999zm2.493 8.574a.5.5 0 0 1-.411.575c-.712.118-1.28.295-1.655.493a1.319 1.319 0 0 0-.37.265.301.301 0 0 0-.057.09V14l.002.008a.147.147 0 0 0 .016.033.617.617 0 0 0 .145.15c.165.13.435.27.813.395.751.25 1.82.414 3.024.414s2.273-.163 3.024-.414c.378-.126.648-.265.813-.395a.619.619 0 0 0 .146-.15.148.148 0 0 0 .015-.033L12 14v-.004a.301.301 0 0 0-.057-.09 1.318 1.318 0 0 0-.37-.264c-.376-.198-.943-.375-1.655-.493a.5.5 0 1 1 .164-.986c.77.127 1.452.328 1.957.594C12.5 13 13 13.4 13 14c0 .426-.26.752-.544.977-.29.228-.68.413-1.116.558-.878.293-2.059.465-3.34.465-1.281 0-2.462-.172-3.34-.465-.436-.145-.826-.33-1.116-.558C3.26 14.752 3 14.426 3 14c0-.599.5-1 .961-1.243.505-.266 1.187-.467 1.957-.594a.5.5 0 0 1 .575.411z"/>
      ...
      <ul className="dropdown-menu" style={{background: '#B2BABB'}}>
        <li><a className='dropdown-item' onClick={() => cambiarFiltro('Aceptado')}>Aceptados</a></li>
        <li><a className='dropdown-item' onClick={() => cambiarFiltro('Pendiente')}>Pendientes</a></li>
        <li><a className='dropdown-item' onClick={() => cambiarFiltro('Denegado')}>Denegados</a></li>
      </ul>
      </div>
      ...
      <div className="col">
        <div className="row row-cols-1 row-cols-sm-2 g-4">
          <div className="col d-flex flex-column gap-2">
            ...
            <svg className="bi" style={{width:"1em", height:"1em"}}><use href="#people-circle"/></svg>
            </div>
            <h6 className="fw-semibold mb-0 text-body-emphasis">Usuario</h6>
            <p className="text-body-secondary">{formulario.username}</p>
          </div>

          <div className="col d-flex flex-column gap-2">
            <div className="feature-icon-small d-inline-flex align-items-center justify-content-center text-bg-primary bg-gradient fs-4 rounded-3">
              <svg className="bi" style={{width:"1em", height:"1em"}}>
                <use href="#geo-fill" />
              </svg>
            </div>
            <h6 className="fw-semibold mb-0 text-body-emphasis">Area</h6>
            <p className="text-body-secondary">{formulario.area}</p>
          </div>
          ...
          <div className="col d-flex flex-column gap-2">
            ...
            <h6 className="fw-semibold mb-0 text-body-emphasis">Empresa</h6>
            <p className="text-body-secondary">{formulario.empresa}</p>
          </div>
